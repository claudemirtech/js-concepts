...
test('Create a new date as string', () => {
	...
	expect(dateUtil.newLocaleDateString()).toBe(actualDate);
});

test('Create a fixed date', () => {
	// we have a string date with 'yyyy-MM-dd'
	const originalDate = '2023-09-10';
	expect(`${dateUtil.toFixedDateTime(originalDate)}`).toBe('Sun Sep 10 2023 12:00:00 GMT-0300 (Horário Padrão de Brasília)');
});

test('Format Date from the string date', () => {
	// we have a string date with 'yyyy-MM-dd'
	const originalDate = '2023-09-10';
	expect(`${dateUtil.formatDate(originalDate)}`).toBe('10/09/2023');
});